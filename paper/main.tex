...
% Every measured figure quoted in the prose, generated from benchmarks/raw/
% by scripts/generate_report.py. Typing one in by hand is how the abstract
% came to claim a wall-time improvement that two re-measurements ago had
% stopped clearing the experiment's own noise floor.
\input{tables/measured.tex}
...
shared-memory MPI, communication-aware placement removes \aegisqQftTraffic{} of
measured MPI traffic for a \aegisqQubits-qubit quantum Fourier transform on
\aegisqRanks{} ranks and \aegisqGroverTraffic{} for a Grover circuit, while
leaving a GHZ chain unchanged; the cost model predicted the byte count exactly
in all \aegisqExactPredictions{} of \aegisqPredictionConfigs{} distributed
configurations measured. Wall time on this host is another matter: the circuits
the optimiser cannot improve still vary by \aegisqWallNoiseFloor{} between
placements, so we report the traffic reduction as the result and treat
wall-time differences below that floor as unresolved. We additionally protect job submission
and result provenance with the NIST-standardised post-quantum primitives
ML-KEM-768 and ML-DSA-65, and measure that overhead: \aegisqEnvelopeOverhead{} of the runtime
of the heaviest job in our suite, of which only $\aegisqLatticeMicros\,\mu$s is lattice
...
This report describes a system built around two questions.
...
$O(\sqrt{N})$ oracle queries and that Shor's algorithm factors 15 and 21 are
textbook results; we reproduce them to show the simulator gets them right,
not as findings.
...
\cite{deraedt2007,quest2019,cuquantum}. Reordering qubits so that the
expensive ones become local is not new: qHiPSTER \cite{smelyanskiy2016} and
its successor the Intel Quantum Simulator \cite{guerreschi2020} reorder to
keep communicating gates on local positions, and H{\"a}ner and Steiger
\cite{haner2017} combine qubit ordering with gate fusion and scheduling in
the 45-qubit simulation that defines the scale of this problem. Those systems
choose an ordering as a means to an end and report the end.

What this work adds is not the idea but its treatment as an object of
measurement. The permutation is explicit in the runtime rather than implicit
in a scheduling pass; an analytical cost model predicts the consequence of a
given placement in bytes and messages \emph{before} it is run; the search over
placements is separated from the runtime that executes them, so the model's
prediction and the runtime's counters can be compared on every configuration;
and the placement lever is measured against gate fusion and windowed
re-mapping under one harness, which is what shows whether the three overlap.
We make no claim to a faster simulator than the systems above, and
Section~\ref{sec:limitations} says why no such comparison is attempted here.
...
\begin{enumerate}
\item A \textbf{front end} providing a circuit IR over twelve input gates
  and one fused arbitrary single-qubit unitary, which only the compiler emits
...
\item A \textbf{compiler} comprising an analytical communication cost model, a
  static placement search, a gate-fusion pass and a windowed placement
  planner.
...
\subsection{Calibrating the harness}
\label{sec:calibration}

Byte counts are exact: they come from counters incremented where the runtime
hands a buffer to MPI. Wall times are not, and on a single laptop the question
of how small a difference this setup can see has to be answered before any
timing is quoted. We answer it by measurement rather than assertion.

For each configuration the identical run is launched twice and the apparent
change $(t_2 - t_1)/t_1$ recorded, $\aegisqNullTrials$ times over. Nothing
differs between the two launches, so every value is zero by construction and
what the clock reports is the instrument. It reports a great deal: the largest
apparent change with no change made is $\aegisqWorstNull$, and even the
quietest configuration --- $\aegisqBestNullCircuit$, the longest-running
circuit in the set --- scatters by $\aegisqBestNull$. This is an A/A test, and
it settles an earlier version of Table~\ref{tab:mapping}, which reported a
wall-time change of comparable size for a circuit whose byte count the
optimiser had left exactly unchanged.

We define the \emph{resolution} of a configuration as the largest apparent
change over its null trials. An observed effect exceeding all $n$ of them has
probability at most $1/(n+1)$ of arising by chance if the two conditions were
exchangeable, which for $n = \aegisqNullTrials$ is $p \le
\aegisqNullPBound$. It is a weak bound and we state it rather than imply a
stronger one. A wall-time change is reported as \emph{resolved} only if it
clears its own configuration's resolution and its repeat range does not
overlap the baseline's; the resolution is measured per circuit family, because
a GHZ chain finishing in eight milliseconds and a Grover circuit running for
half a second are not equally timeable.

The shape of the null also determines the right estimator. Interference on
this machine is one-sided --- nothing schedules a launch to run faster than an
uncontended one --- so launch times are right-skewed with a long upper tail,
and the minimum over independent launches converges on the uncontended
runtime while a mean chases the tail. Taking each arm as the minimum over $k$
launches rather than over three repeats inside one launch moves the median
resolution from $\aegisqResolutionOne$ at $k=1$ to $\aegisqResolutionThree$
at $k=3$ and $\aegisqResolutionFive$ at $k=5$, estimated by resampling the
measured launch times. The sweep supports this through
\texttt{-{}-launches}.

\aegisqWallVerdict. A second check, which assumes almost nothing, points the
same way: the direction of every treated configuration agrees, and under a
null in which each sign is an independent coin flip that carries
$p = 2^{-\aegisqTreatedCount}$. Direction is far cheaper to measure than
magnitude, and it is the part worth leaning on. The traffic reduction remains
the primary result, because it is counted rather than timed.
...
seeded random circuits per run. The suite is run in full before every
measurement sweep; its size is reported by the test runner rather than quoted
here, since a number typed into prose stops being true the first time a test
is added.
...
removes \aegisqQftTraffic{} of the traffic. \aegisqQftWallClause{} (see
Section~\ref{sec:calibration}). Grover's dense
...
Wall time does not track bytes proportionally. The Ising chain sheds
\aegisqIsingTraffic{} of its traffic with no resolvable time improvement, which
is what one expects when communication is not the bottleneck --- here,
shared-memory MPI on one socket, where a ``transfer'' is a memory copy.

The cost model predicted the byte count and the message count \emph{exactly} in
all \aegisqPredictionConfigs{} distributed configurations measured.

\subsection{When is the search necessary?}

The cost rules are worth reading for what they depend on. A non-diagonal
single-qubit gate costs a full shard if \emph{its} qubit is global; a
\texttt{cx} costs half a shard if \emph{its target} is global, irrespective
of the control, because the byte volume is the same either way. Both depend on
one qubit's membership of the global set. \texttt{swap} is the exception: it
costs if \emph{either} operand is global, and a disjunction is not a sum.

It follows that for a circuit with no \texttt{swap} gates the objective is
linear in the indicator of the global set, and the optimum is the $p$ qubits
with the smallest individual costs --- obtainable by sorting, with no search
at all. The mapper detects this case and reports it as such rather than
running a search it does not need.

Where \texttt{swap} gates appear the problem is combinatorial and the greedy
fallback could settle for a local optimum. Across 99 sampled configurations at
4, 8 and 16 ranks, 93 of which required the heuristic, it matched the
exhaustive optimum in every case while running $3$--$40\times$ faster. That is
an empirical result on these circuit families rather than a guarantee, and it
is reported as one.
...
\subsection{A third lever: windowed placement}

A single assignment is the right answer only for a circuit whose communication
structure is uniform. We split a circuit into windows, find the best
assignment for each, and switch when the saving on a window exceeds the cost
of the switch --- half a shard per rank for each qubit that changes group.

A plan is executed as a circuit rewrite rather than a runtime feature: gates
are re-expressed on slots, a change of assignment becomes SWAP gates between
slots, and the resulting traffic is measured by the same instrumentation as
every other gate. The final permutation is undone by relabelling results,
which is free classical bookkeeping, exactly as the runtime mapping is for a
static assignment.

Two details determine whether the comparison means anything. First, the
baseline must be the best \emph{static} assignment, not the default one;
measuring against the default would credit windowing with the gain ordinary
placement already delivers. Second, the plan must not be charged for
restoration that the static baseline gets for free. Both were wrong in our
first implementation, and both showed up as windowing apparently losing to
static placement on circuits where it plainly should not.

Measured at \aegisqRanks{} ranks, windowed placement beats the best single
assignment on four of five families: \aegisqQftWindowed{} of baseline traffic
removed for the QFT against \aegisqQftPlacement{} static, and
\aegisqGroverWindowed{} against \aegisqGroverPlacement{} for Grover. On GHZ,
which has no phase structure, the planner declines to switch and the two
coincide. For random circuits the best result comes from placement and fusion
together (\aegisqRandomBoth) rather than from windowing
(\aegisqRandomWindowed), so no single lever dominates.

\input{tables/strong_scaling.tex}
...
\subsection{Ordering the local qubits}
\label{sec:localorder}

The cost model decides which qubits are global, and is silent about the rest:
two placements with the same global set move byte for byte the same traffic,
so the local order was left at the identity. The kernel sweep of
Section~\ref{sec:kernels} says that order is not free in time. The most
position-sensitive kernel, \aegisqWorstPositionKernel, varies by
\aegisqWorstPositionSpread{} across local target positions; the least,
\aegisqBestPositionKernel, by \aegisqBestPositionSpread. Since reordering
locally costs no traffic, choosing the order well is free.

We do so by counting how often each qubit is the position-sensitive operand of
each opcode --- the target for two-qubit gates, which is what the sweep varies
--- pricing every (qubit, position) pair from the measured table, and solving
the resulting assignment. The objective is a sum of per-qubit terms, so this
is a linear assignment problem and the Hungarian algorithm returns the exact
optimum.

The result is a negative one on this circuit mix, and we report it as such.
Predicted savings in local kernel time are $0.4\%$ for the QFT, $0.0\%$ for
Ising, $1.6\%$ for Grover and $2.9\%$ for random circuits --- all below the
resolution established in Section~\ref{sec:calibration}, so none is claimed
as a speedup. These circuits are dominated by \texttt{rz}, \texttt{cx} and
\texttt{h}, whose position spread is small, and \texttt{cz} is rare in all of
them. On a synthetic \texttt{cz}-heavy circuit the prediction rises to about
$20\%$ and the chosen order puts the \texttt{cz} targets on the fast
positions, which is the behaviour the kernel data predicts. The freedom is
real and now exploited exactly; on this benchmark mix there is simply little
to exploit.

\subsection{Are the local kernels efficient?}
\label{sec:kernels}

Wall time says whether a run was fast, not whether the implementation was
good. State-vector simulation is bandwidth-bound, so each kernel is compared
against what the same machine achieves on an in-place scale of an array of the
same type, compiled with the same flags and threaded the same way. The
in-place shape is the comparable one: a gate reads and writes a single array,
which is not the traffic shape of a two-array copy.

At 8 threads and 24 qubits the kernels that sweep the whole state --- a
general single-qubit gate and a diagonal one --- reach $98\%$ of that
reference. They are memory-bound, and there is little left to win. The kernels
that touch only part of the state (\texttt{cx}, \texttt{cz}, \texttt{swap})
plateau between $52\%$ and $59\%$: they read and write a strided fraction of
the array and leave roughly half the available bandwidth unused. That is a
concrete optimisation target rather than an unexplained gap.

The same sweep tests an assumption the cost model makes. The model treats the
local/global distinction as the only thing that matters and ignores which
local position a qubit occupies. Measured across target positions and
every thread count, the least position-sensitive kernel is
\aegisqBestPositionKernel, whose bandwidth still varies by
\aegisqBestPositionSpread{} in its worst case, and the most sensitive is
\aegisqWorstPositionKernel{} at \aegisqWorstPositionSpread. Each kernel is
summarised by its worst thread count rather than its best, since quoting the
most favourable row would make the assumption look sounder than it is. The model is therefore accurate about network traffic and
incomplete about local cost.

\subsection{RQ2: cost of the post-quantum layer}

\input{tables/pqc.tex}

Table~\ref{tab:pqc} gives primitive costs. End to end, packing a 20-qubit job
bundle takes $\aegisqPackMs$\,ms and verifying, decrypting and opening it
$\aegisqVerifyMs$\,ms, with $\aegisqEnvelopeBytes$ bytes of envelope overhead
independent of circuit size. Against the heaviest job in our suite
(\aegisqHeaviestCircuit, $\aegisqHeaviestMs$\,ms,
$\aegisqHeaviestMiB$\,MiB of MPI traffic) that is \aegisqEnvelopeOverhead{}
of runtime.

The decomposition matters more than the total: only $\aegisqLatticeMicros\,\mu$s of that is
...
\subsection{Motivation: why post-quantum cryptography}

\input{tables/grover.tex}
...
The simulator is exact and noiseless, accepts twelve gates in its input
language --- plus a fused arbitrary single-qubit unitary that only the fusion
pass produces, and that the OpenQASM reader deliberately rejects --- and
supports terminal measurement only, and requires power-of-two rank counts. The cost model counts
bytes and messages and does not model topology, congestion, or overlap of
computation with communication. Windowed placement re-maps between circuit
segments, but its windows are fixed-width and chosen by a dynamic program over
a cost approximation rather than by the exact transition cost.
...
Three directions follow directly from what is measured here. \emph{Cluster
measurements}: the same experiments on
...
account for the interconnect a job is actually running on. \emph{A local term
in the cost model}: the measured spread in \texttt{cz} bandwidth across local
target positions is invisible to a model that counts only network bytes, so a
placement can be network-optimal and locally poor;
Section~\ref{sec:localorder} takes a first step and finds little to gain on
this circuit mix.

An earlier draft listed joint search over placement and fusion here. It does
not belong: fusion rewrites the circuit, so the placement must be chosen
afterwards --- measured on random circuits, choosing it first costs up to
$12\%$ more traffic --- and the pipeline already runs in that order. Fusion's
only tunable is the minimum run length, and its most aggressive setting is
never beaten, so there is no joint space left to search. Both properties are
asserted in the test suite rather than left as an expectation.
